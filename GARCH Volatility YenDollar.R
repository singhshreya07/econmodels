...
#JAPENESE YEN PER DOLLAR, MONTHLY 1974-2019

#IMPORT DATA AND CHECK
data <- read.csv("C:\\Users\\HP\\Desktop\\Ecotrix Projects\\JPYUSD.csv")
tail(data)
...
#set e as log difference, which will approximate the percentage changes
e=diff(log(E))

#plot w/zero line
plot(e, ylab, "", xlab ="")
abline(h=0, col="dark grey", lty=3, )

# Some Box-Jenkins, starting with an AR(1) process
acf(e)
#it does decay gradually
pacf(e)
ar1<-arima(e,c(1,0,0))
ar1
...
#INTERPRETATION: there is periods of high volatility during the Asian crisis and then the Great Recession around 1990s
...
#you can have a positive occurence shock like currency appreciation (dollar) which is different from a negative shock occurence (yen depreciating)
...
#gamma and beta are significant 
#if you see the robust standard errors, gamma1 is not significant
#Standard GARCH model seems better, and has aslightly better AIC
...
cor(vole,vole2)
#highly likely similar

#Plot them both together
ts.plot(vole,vole2,col=c("green","red"),xlab="") 
#SGARCH tends to be affected at higher extremes but EGARCH does not
...
voldif<-vole-vole2
plot(voldif,xlab="")
#Could not see any depreciations and appreciations that drives the exchange rates
...
abline(h=0,lty=2)
abline(v=0,lty=2)
#No obvious relationship can be established 
#
